...
#include "launch.cuh"
#include "utils.cuh"
#ifdef IBGDA_MLX_SUPPORT
#include "ibgda_device.cuh"
#else
#include "ibgda_device_bnxt.cuh"
#endif
...
#else
                    nvshmemi_ibgda_rma_p(reinterpret_cast<int*>(rdma_channel_tail.buffer(rdma_rank)), num_tokens_to_issue,
                                         translate_dst_rdma_rank<kLowLatencyMode>(dst_rdma_rank, nvl_rank), channel_id, dst_rdma_rank == rdma_rank);
                    nvshmemi_ibgda_prepare_recvs(translate_dst_rdma_rank<kLowLatencyMode>(dst_rdma_rank, nvl_rank), channel_id);
#endif
                }
                __syncwarp();
            }
...
#else
                nvshmemi_ibgda_rma_p(reinterpret_cast<int*>(rdma_channel_tail.buffer(rdma_rank)), min_head - last_head,
                                     translate_dst_rdma_rank<kLowLatencyMode>(lane_id, nvl_rank), channel_id + num_channels, lane_id == rdma_rank);
                nvshmemi_ibgda_prepare_recvs(translate_dst_rdma_rank<kLowLatencyMode>(lane_id, nvl_rank), channel_id + num_channels);
#endif
                last_head = min_head;
            }
                ...
                if (sub_warp_id == kNumWarpsPerForwarder - 1) {
                    ...
                    if (lane_id == 0) {
                        ...
                        nvshmemi_ibgda_amo_nonfetch_add(rdma_channel_tail.buffer(rdma_rank), num_chunked_tokens,
                                                        translate_dst_rdma_rank<kLowLatencyMode>(dst_rdma_rank, nvl_rank), channel_id, dst_rdma_rank == rdma_rank);
#else
                        nvshmemi_ibgda_rma_p(reinterpret_cast<int*>(rdma_channel_tail.buffer(rdma_rank)), num_chunked_tokens,
                                             translate_dst_rdma_rank<kLowLatencyMode>(dst_rdma_rank, nvl_rank), channel_id, dst_rdma_rank == rdma_rank);
                        nvshmemi_ibgda_prepare_recvs(translate_dst_rdma_rank<kLowLatencyMode>(dst_rdma_rank, nvl_rank), channel_id);
#endif
                    }
...
#else
                        nvshmemi_ibgda_rma_p(reinterpret_cast<int*>(rdma_channel_tail.buffer(rdma_rank)), min_head - last_rdma_head,
                                             translate_dst_rdma_rank<kLowLatencyMode>(dst_rdma_rank, nvl_rank), channel_id + num_channels, dst_rdma_rank == rdma_rank);
                        nvshmemi_ibgda_prepare_recvs(translate_dst_rdma_rank<kLowLatencyMode>(lane_id, nvl_rank), channel_id + num_channels);
#endif
