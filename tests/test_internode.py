...
def test_main(num_sms: int, local_rank: int, num_local_ranks: int, num_ranks: int, num_nodes: int, rank: int, buffer: deep_ep.Buffer, group: dist.ProcessGroup):
    ...
    print(f'[config] local_rank={local_rank}, num_local_ranks={num_local_ranks}, num_ranks={num_ranks}, num_nodes={num_nodes}', flush=True)
...
if __name__ == '__main__':
    num_processes = int(os.getenv('DEEPEP_TEST_GPU_PER_NODE', '1'))
    num_nodes = int(os.getenv('DEEPEP_TEST_NUM_OF_NODES', '2'))
