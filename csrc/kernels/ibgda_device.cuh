...
__device__ static __forceinline__ void*
ibgda_get_rqe_ptr(nvshmemi_ibgda_device_qp_t* qp, uint16_t wqe_idx) {
    uint16_t cnt = qp->rx_wq.nwqes;
    uint16_t idx = wqe_idx & (cnt - 1);
    return reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(qp->rx_wq.wqe) + (idx << MLX5_SEND_WQE_SHIFT));
}
