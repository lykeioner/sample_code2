...
if __name__ == '__main__':
    ...
    # AMO_OPS
    if int(os.getenv('IBGDA_USE_AMO_OPS', '0')):
        cxx_flags.append('-DIBGDA_USE_AMO_OPS')
        nvcc_flags.append('-DIBGDA_USE_AMO_OPS')

    # GPU per node
    num_gpu_per_node = int(os.getenv('DEEPEP_TEST_GPU_PER_NODE', '1'))
    cxx_flags.append(f'-DNUM_GPU_PER_NODE={num_gpu_per_node}')
    nvcc_flags.append(f'-DNUM_GPU_PER_NODE={num_gpu_per_node}')

    # Enable IBGDA MLX_SUPPORT
    if int(os.getenv('IBGDA_MLX_SUPPORT', '1')):
        cxx_flags.append('-DIBGDA_MLX_SUPPORT')
        nvcc_flags.append('-DIBGDA_MLX_SUPPORT')

    if int(os.getenv('NVSHMEM_DEBUG', '1')):
        cxx_flags.append('-DNVSHMEM_IBGDA_DEBUG')
        nvcc_flags.append('-DNVSHMEM_IBGDA_DEBUG')
    ...
    if int(os.getenv('IBGDA_MLX_SUPPORT', '1')):
        print(f' > Building for MLX CUDA Plugin')
    else:
        print(f' > Building for BNXT CUDA Plugin')
    print()
